...
function Navbar() {
    ...
    function useri(type) {
        /*  */
        if (type) {
            return signed ? <img onClick={() => navigate("/user")} id="usrIcon" src={user.photoURL || "https://tinyurl.com/5kub7nce"} /> : <AccountCircleOutlinedIcon onClick={() => navigate("/user")} fontSize="large" />;
        } else {
            return signed ? (
                <ListItem id="pfpsec">
                    <Link to="/user">
                        <img onClick={() => navigate("/user")} id="pfp" src={user.photoURL || "https://tinyurl.com/5kub7nce"} />
                    </Link>
                </ListItem>
            ) : (
                ""
    ...
    return (
        <>
            <header>
                ...
                <Drawer id="drawer" anchor="right" open={open} onClose={toggleDrawer(false)} ModalProps={{ keepMounted: true }}>
                    ...
                    <ListItem onClick={toggleDrawer(false)}>{teste()}</ListItem>
                </Drawer>
            </header>
        </>
    );
